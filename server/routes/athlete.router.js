...
router.get("/", rejectUnauthenticated, (req, res) => {
  ...
  pool
    .query(queryText, [req.user.org_id])
    .then((result) => {
...
router.get("/athletesInLane/:lane", rejectUnauthenticated, (req, res) => {
  ...
  pool
    .query(queryText, [req.params.lane, req.user.org_id])
    .then((result) => {
      res.send(result.rows);
    })
    .catch((error) => {
...
router.post("/", rejectUnauthenticated, (req, res) => {
  ...
  let queryText =
    "INSERT INTO athlete (athlete_name, lane_number, year, org_id) VALUES($1,$2,$3,$4)";
  pool
    .query(queryText, [desc, img, usr, req.user.org_id])
    .then((result) => {
      res.sendStatus(202);
    })
    .catch((error) => {
...
router.delete("/:id/:org_id", rejectUnauthenticated, (req, res) => {
  if (
    ...
    (req.user.org_id != req.params.org_id && req.user.auth_level < 6)
  ) {
    ...
    pool
      ...
      .then((result) => {
        pool
          .query("DELETE FROM times WHERE athlete_id=$1", [req.params.id])
          .then((result) => {
            res.sendStatus(203);
          })
          .catch((error) => {
...
router.put("/", rejectUnauthenticated, (req, res) => {
  ...
  let queryText =
    "UPDATE athlete SET active=$1, year=$2 ,lane_number=$3 where id=$4";
  pool
    .query(queryText, [active, year, lane, id])
    .then((result) => {
      res.sendStatus(203);
    })
    .catch((error) => {
